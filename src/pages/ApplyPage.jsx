...
const LanguageContext = createContext({ t: (value) => value });
const ES = {
    'Personal info': 'Información personal', 'Availability & eligibility': 'Disponibilidad y elegibilidad', 'Cleaning experience': 'Experiencia de limpieza', 'Work history': 'Historial laboral', 'References': 'Referencias', 'Review & submit': 'Revisar y enviar',
    'Step 1 of 6': 'Paso 1 de 6', 'Step 2 of 6': 'Paso 2 de 6', 'Step 3 of 6': 'Paso 3 de 6', 'Step 4 of 6': 'Paso 4 de 6', 'Step 5 of 6': 'Paso 5 de 6', 'Step 6 of 6': 'Paso 6 de 6',
    'Tell us how to reach you.': 'Cuéntenos cómo podemos comunicarnos con usted.', 'Share your work availability.': 'Comparta su disponibilidad laboral.', 'Show us your experience.': 'Muéstrenos su experiencia.', 'Add your previous employers.': 'Agregue sus empleadores anteriores.', 'Add professional references.': 'Agregue referencias profesionales.', 'Review and send your application.': 'Revise y envíe su solicitud.',
    'Use the legal name and contact information you want our hiring team to use.': 'Use el nombre legal y la información de contacto que desea que use nuestro equipo de contratación.', 'This helps us match you with cleaning schedules across our service area.': 'Esto nos ayuda a relacionarlo con horarios de limpieza en nuestra área de servicio.', 'Tell us about the work settings, duties, and equipment you know.': 'Cuéntenos sobre los lugares de trabajo, tareas y equipos que conoce.', 'Please provide exactly two previous-employer entries.': 'Proporcione exactamente dos empleadores anteriores.', 'Please provide exactly two people who can speak to your work.': 'Proporcione exactamente dos personas que puedan hablar sobre su trabajo.', 'Your application is sent securely to the Master Commercial Clean hiring team.': 'Su solicitud se envía de forma segura al equipo de contratación de Master Commercial Clean.',
    'Legal first name *': 'Nombre legal *', 'Legal last name *': 'Apellido legal *', 'Preferred name': 'Nombre preferido', 'Email address *': 'Correo electrónico *', 'Phone number *': 'Número de teléfono *', 'Preferred contact method *': 'Método de contacto preferido *', 'Street address *': 'Dirección *', 'City *': 'Ciudad *', 'State *': 'Estado *', 'ZIP code *': 'Código postal *', 'Earliest start date *': 'Fecha más temprana para comenzar *', 'Employment preference *': 'Preferencia de empleo *', 'Desired weekly hours *': 'Horas semanales deseadas *', 'Days available *': 'Días disponibles *', 'Available times *': 'Horarios disponibles *',
    'Available for evenings? *': '¿Disponible por las tardes/noches? *', 'Available for weekends? *': '¿Disponible los fines de semana? *', 'Do you have reliable transportation? *': '¿Tiene transporte confiable? *', 'Can you travel throughout our service area? *': '¿Puede viajar por toda nuestra área de servicio? *', 'Are you at least 18 years old? *': '¿Tiene al menos 18 años? *', 'Janitorial or commercial cleaning experience? *': '¿Tiene experiencia en limpieza comercial o de conserjería? *', 'Years of experience *': 'Años de experiencia *', 'Prior work settings *': 'Ambientes de trabajo anteriores *', 'Duties performed *': 'Tareas realizadas *', 'Floor and equipment experience *': 'Experiencia con pisos y equipos *', 'Relevant certifications': 'Certificaciones relevantes', 'Optional résumé': 'Currículum opcional',
    'Company *': 'Empresa *', 'Role *': 'Puesto *', 'Supervisor *': 'Supervisor *', 'Supervisor phone *': 'Teléfono del supervisor *', 'Start date *': 'Fecha de inicio *', 'End date *': 'Fecha de finalización *', 'Duties *': 'Tareas *', 'Reason for leaving *': 'Motivo de salida *', 'May we contact this employer? *': '¿Podemos contactar a este empleador? *', 'Full name *': 'Nombre completo *', 'Relationship *': 'Relación *', 'Phone *': 'Teléfono *', 'Email *': 'Correo electrónico *', 'Years known *': 'Años de conocerle *', 'May we contact this reference? *': '¿Podemos contactar a esta referencia? *',
    'Additional comments': 'Comentarios adicionales', 'Submission date': 'Fecha de envío', 'Type your full legal name as your signature *': 'Escriba su nombre legal completo como firma *', 'Application readiness': 'Estado de la solicitud', 'sections complete': 'secciones completadas', 'Review each section in the left menu. A check mark means that section is ready to submit.': 'Revise cada sección en el menú de la izquierda. Una marca indica que la sección está lista para enviar.',
    'Full-time': 'Tiempo completo', 'Part-time': 'Medio tiempo', 'Either': 'Cualquiera', 'Phone': 'Teléfono', 'Email': 'Correo electrónico', 'Yes': 'Sí', 'No': 'No', 'Select one': 'Seleccione una', 'Optional': 'Opcional', 'Employer': 'Empleador', 'Reference': 'Referencia', 'Most recent role': 'Puesto más reciente', 'Previous role': 'Puesto anterior', 'Professional reference': 'Referencia profesional', 'Previous': 'Anterior', 'Next': 'Siguiente', 'Submit application': 'Enviar solicitud', 'Submitting…': 'Enviando…',
    'I understand that employment authorization will be verified after a conditional offer of employment. *': 'Entiendo que la autorización de empleo se verificará después de una oferta de empleo condicional. *', 'I certify that the information in this application is accurate and complete. *': 'Certifico que la información de esta solicitud es exacta y completa. *', 'I authorize Master Commercial Clean to verify my employment history and professional references. *': 'Autorizo a Master Commercial Clean a verificar mi historial laboral y referencias profesionales. *', 'I consent to the collection and use of this information for employment consideration. *': 'Autorizo la recopilación y el uso de esta información para fines de consideración de empleo. *', 'I-9 and W-4 next steps remain locked until the hiring team approves your application. We never ask for SSNs, tax values, or identity-document details in this application.': 'Los siguientes pasos del I-9 y W-4 permanecen bloqueados hasta que el equipo de contratación apruebe su solicitud. Nunca solicitamos números de Seguro Social, datos fiscales ni documentos de identidad en esta solicitud.',
    'Employment / job application': 'Solicitud de empleo', 'Save your place automatically in this browser. You can return to mastercleanhq.com/apply and continue.': 'Su progreso se guarda automáticamente en este navegador. Puede volver a mastercleanhq.com/apply y continuar.', 'Application': 'Solicitud', 'Next steps': 'Próximos pasos', 'Locked': 'Bloqueado', 'Unlocked only after the hiring team approves your application.': 'Se desbloquea únicamente después de que el equipo de contratación apruebe su solicitud.',
};
const initialForm = () => ({
...
function Field({ label, children, hint, wide = false }) {
    ...
    return <label className={`apply-field${wide ? ' apply-field-wide' : ''}`}><span>{t(label)}</span>{children}{hint && <small>{t(hint)}</small>}</label>;
}

function YesNo({ label, value, onChange, hint, name }) {
    const { t } = useContext(LanguageContext);
    return <fieldset className="apply-choice-field"><legend>{t(label)}</legend>{hint && <small>{t(hint)}</small>}<div className="apply-choice-row">
        <label><input name={name} type="radio" checked={value === 'yes'} onChange={() => onChange('yes')} /> {t('Yes')}</label>
        <label><input name={name} type="radio" checked={value === 'no'} onChange={() => onChange('no')} /> {t('No')}</label>
    </div></fieldset>;
}

function LanguageToggle({ language, onChange }) {
    return <div className="apply-language-toggle" aria-label="Application language">
        <button className="apply-secondary-button" type="button" aria-pressed={language === 'en'} onClick={() => onChange('en')}>English</button>
        <button className="apply-secondary-button" type="button" aria-pressed={language === 'es'} onClick={() => onChange('es')}>Español</button>
    </div>;
}
...
const ApplyPage = () => {
    ...
    const t = useCallback((value) => language === 'es' ? (ES[value] || value) : value, [language]);
    ...
    if (!started) return <LanguageContext.Provider value={{ t }}><div className="apply-page apply-entry-page" lang={language}>{seo}<section className="apply-entry-hero"><div className="container"><LanguageToggle language={language} onChange={setLanguage} /><div><span>Careers at Master Commercial Clean</span><h1>{language === 'es' ? <>Trabaje con un equipo que se enorgullece de <em>cada detalle.</em></> : <>Work with a team that takes <em>pride</em> in every detail.</>}</h1><p>{language === 'es' ? 'Inicie una solicitud segura, guarde su progreso en este navegador y vuelva cuando esté listo.' : 'Start a secure application, save your progress in this browser, and return whenever you are ready.'}</p><button className="apply-button" type="button" onClick={startApplication}>{language === 'es' ? 'Iniciar solicitud' : 'Start application process'} <span>→</span></button></div><aside><strong>{language === 'es' ? 'Qué esperar' : 'What to expect'}</strong><ul><li>{language === 'es' ? 'Seis secciones claras de solicitud' : 'Six clear application sections'}</li><li>{language === 'es' ? 'Su trabajo en curso se guarda en este navegador' : 'Your in-progress work is saved in this browser'}</li><li>{language === 'es' ? 'I-9 y W-4 se desbloquean después de la aprobación' : 'I-9 and W-4 unlock only after approval'}</li></ul></aside></div></section><Breadcrumbs items={[{ name: 'Home', path: '/' }, { name: 'Apply', path: '/apply' }]} /></div></LanguageContext.Provider>;

    const step = STEP_META[activeStep];
    return <LanguageContext.Provider value={{ t }}><div className="apply-page apply-process-page" lang={language}>{seo}<Breadcrumbs items={[{ name: 'Home', path: '/' }, { name: 'Apply', path: '/apply' }]} /><main className="container apply-process-shell"><header className="apply-process-title"><LanguageToggle language={language} onChange={setLanguage} /><span>{t('Employment / job application')}</span><h1>Master Commercial Clean</h1><p>{t('Save your place automatically in this browser. You can return to mastercleanhq.com/apply and continue.')}</p></header>{restoreNotice && <div className="apply-restore-notice" role="status"><span>✓</span><p>Your progress has been restored. Your résumé file is not saved by your browser for privacy, so select it again before submitting if needed.</p><button type="button" onClick={() => setRestoreNotice(false)}>Dismiss</button><button type="button" onClick={startOver}>Start over</button></div>}<div className="apply-process-grid"><aside className="apply-progress-nav"><div className="apply-progress-group"><header><span>{t('Application')}</span><strong>{completeCount}/{STEP_META.length}</strong></header>{STEP_META.map((item, index) => <button key={item.id} className={`apply-progress-item ${index === activeStep ? 'active' : ''} ${complete[item.id] ? 'complete' : ''}`} type="button" onClick={() => goToStep(index)}><i>{complete[item.id] ? '✓' : index + 1}</i><span>{t(item.label)}</span></button>)}</div><div className="apply-progress-group"><header><span>{t('Next steps')}</span><strong>{t('Locked')}</strong></header><div className="apply-progress-item locked"><i>⌕</i><span>Official Form I-9</span></div><div className="apply-progress-item locked"><i>⌕</i><span>Official Form W-4</span></div><p className="apply-nav-note">{t('Unlocked only after the hiring team approves your application.')}</p></div></aside><form className="apply-wizard-card" onSubmit={submit} noValidate><div className="apply-step-heading"><span>{t(step.kicker)}</span><h2>{t(step.title)}</h2><p>{t(step.copy)}</p></div>{renderStep()}<div className="apply-honeypot" aria-hidden="true"><label>Company website<input tabIndex="-1" autoComplete="off" value={form.company_website} onChange={(event) => update('company_website', event.target.value)} /></label></div>{message.text && <p aria-live="polite" className={message.kind === 'error' ? 'apply-message apply-message-error' : 'apply-message'}>{message.text}</p>}<div className="apply-wizard-actions"><button className="apply-secondary-button" type="button" disabled={activeStep === 0 || message.kind === 'loading'} onClick={() => goToStep(Math.max(activeStep - 1, 0))}>← {t('Previous')}</button>{activeStep < STEP_META.length - 1 ? <button className="apply-button" type="button" onClick={goNext}>{t('Next')} <span>→</span></button> : <button className="apply-button" disabled={message.kind === 'loading'} type="submit">{t(message.kind === 'loading' ? 'Submitting…' : 'Submit application')} <span>→</span></button>}</div></form></div></main></div></LanguageContext.Provider>;
};
